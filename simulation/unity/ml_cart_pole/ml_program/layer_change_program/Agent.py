...
class DQNAgent:
    ...
    def init_model(self):
        ...
        with tf.name_scope('fc1'):
            self.W_fc1 = tf.Variable(tf.truncated_normal([FRAMES*INPUTS, 9], stddev=0.01), name="W_fc1")
            self.b_fc1 = tf.Variable(tf.zeros([9]), name="b_fc1")
            h_fc1 = tf.nn.relu(tf.matmul(x_flat, self.W_fc1) + self.b_fc1)

        # hidden layer. fully connected (20)
        with tf.name_scope('fc2'):
            self.W_fc2 = tf.Variable(tf.truncated_normal([9, 3], stddev=0.01), name="W_fc2")
            self.b_fc2 = tf.Variable(tf.zeros([3]),  name="b_fc2")
            self.h_fc2 = tf.nn.relu(tf.matmul(h_fc1, self.W_fc2) + self.b_fc2)

        # output layer. fully connected (3)
        with tf.name_scope('output'):
            # output layer (n_actions)
            self.W_out = tf.Variable(tf.truncated_normal([3, self.n_actions], stddev=0.01), name="W_out")
