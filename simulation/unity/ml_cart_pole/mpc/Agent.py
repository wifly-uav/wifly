...
class DQNAgent:
    ...
    def init_model(self):
        ...
        with tf.name_scope('fc1'):
            W_fc1 = tf.Variable(tf.truncated_normal([FRAMES*INPUTS, 20], stddev=0.01), name="W_fc1")
            b_fc1 = tf.Variable(tf.zeros([20]), name="b_fc1")
            h_fc1 = tf.nn.relu(tf.matmul(x_flat, W_fc1) + b_fc1)

        # hidden layer. fully connected (20)
        with tf.name_scope('fc2'):
            W_fc2 = tf.Variable(tf.truncated_normal([20, 20], stddev=0.01), name="W_fc2")
            b_fc2 = tf.Variable(tf.zeros([20]),  name="b_fc2")
            h_fc2 = tf.nn.relu(tf.matmul(h_fc1, W_fc2) + b_fc2)

        # output layer. fully connected (3)
        with tf.name_scope('output'):
            # output layer (n_actions)
            W_out = tf.Variable(tf.truncated_normal([20, self.n_actions], stddev=0.01), name="W_out")
            b_out = tf.Variable(tf.zeros([self.n_actions]), name="b_out")
