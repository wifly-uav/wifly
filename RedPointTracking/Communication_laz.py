...
KEYS_LOG = ["Slope", "Pitch", "servo", "R-DC", "L-DC", "time"]
#[受信データ , 送信データ , time]
class Communicator():
    """
    マイコンと通信するためのクラス
    ...
    def save_communicate_log(self):
        ...
        通信のログをcsvに出力する関数
        """
        with open('C:/Users/naush/Downloads/result/1103/com_log_{0:%Y%m%d_%H%M%S}'.format(datetime.datetime.now())+ ".csv", 'w') as f:
            writer = csv.writer(f, lineterminator='\n')
            writer.writerows(self.log)

    def receive_from_esp(self, mode = False, byt = 8):
        """
        マイコンからデータを受け取る関数
        ...
        while True:
            ...
            if self.__ser.in_waiting > 0:
                ...
                if len(persed_data) == byt:
                    ...
                    self.time_last_receive = time.time()
                    if (persed_data.pop(0).startswith('S') and persed_data.pop(-1).startswith('E')): # ESPで通信の"S"とかある？
                        if (persed_data[0] != " " or persed_data[0] != "-" or persed_data[0] != ""):                    # persed_data[0] != " " or persed_data[0] != "-"　←元の文 
                            #recieve_time_ = 0
                            #add_log = self.create_log(persed_data, recieve_time)
                            #if mode == True:
                            #    self.log.append(add_log)
                            self.dataset_from_esp = persed_data
                            #print(str(self.dataset_from_laz) + ":" + str(recieve_time))
                            self.__fail_counter = 0
                            return self.dataset_from_esp
                        else:
                            print(persed_data[0])
                            print("no data")
                            print(persed_data)
                ...
                self.send_to_esp(self.__data_sent)
