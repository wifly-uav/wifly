...
class Communicator():
    ...
    def save_communicate_log(self):
        ...
        通信のログをcsvに出力する関数
        """
        with open('C:/Users/naush/Downloads/result/1103/com_log_{0:%Y%m%d_%H%M%S}'.format(datetime.datetime.now())+ ".csv", 'w') as f: # 番号を随時更新
            writer = csv.writer(f, lineterminator='\n')
            writer.writerows(self.log)
            
    def receive_from_esp(self, byt = RECEIVE_BYTE, try_data = [255,254,254,0,0]):
        """
        マイコンからデータを受け取る関数
